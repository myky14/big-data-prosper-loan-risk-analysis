...
Sau khi giải nén, cấu hình biến môi trường HADOOP_HOME để hệ điều hành có thể nhận diện Hadoop.
...
### 4.4. Kiểm tra lại Command Prompt/PowerShell mới và chạy lệnh:
```bash
hadoop version
```
...
Sau khi bổ sung winutils, nhóm kiểm tra lại biến môi trường HADOOP_HOME và đường dẫn %HADOOP_HOME%\bin. Việc này giúp hạn chế các lỗi liên quan đến quyền truy cập, đường dẫn và thao tác file khi Hadoop hoạt động trên Windows.

Sau khi hoàn tất các bước cấu hình trên, Hadoop đã sẵn sàng để format NameNode và khởi động HDFS.
...
```
<img width="1224" height="715" alt="image" src="https://github.com/user-attachments/assets/2562827c-c75b-4042-a06b-66104c6932af" />
...
### 8.1. Start DFS
Chạy lệnh sau để khởi động HDFS:
```bash
start-dfs.sh
```

### 8.2. Start YARN
Chạy lệnh sau để khởi động YARN:
```bash
start-yarn.sh
```

### 8.3. Kiểm tra tiến trình
Sau khi khởi động Hadoop, sử dụng lệnh jps để kiểm tra các tiến trình đang chạy:
```bash
jps
```
Kết quả mong đợi bao gồm các tiến trình như:
- NameNode
- DataNode
- ResourceManager
- NodeManager
...
<img width="905" height="261" alt="image" src="https://github.com/user-attachments/assets/1ceb0118-53e1-4e07-a356-a5e7356e4daa" />
...
- Hadoop được cài đặt và nhận diện qua Command Prompt/PowerShell.
